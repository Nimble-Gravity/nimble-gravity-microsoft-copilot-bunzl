...
assets/lab-data/        — Mixed as of Phase B: 3 real Bunzl sample files (.xlsx/.pptx/.docx,
                          generic knowledge-worker files) now wired into The Loading Dock's
                          (escape-room/) puzzles, alongside the legacy B&B CSVs/data-room still
                          read by control-room's current puzzles; see its README (still
                          B&B-focused — needs a Phase C update)
```
...
app's README. As of Phase B: `escape-room/` ("The Loading Dock," Foundations) is fully
retextured — 4 stations, codes derived from the 3 real Bunzl sample files in `assets/lab-data/`
(`.xlsx`/`.pptx`/`.docx`, generated by the Python tool in `tools/sample-files/`), answer key in
`escape-room/lab-files/loading-dock-briefing.md`. `control-room/` (Advanced) is still the prior
engagement's "The Close Room," with codes derived from the legacy B&B CSVs in `assets/lab-data/`
— retitling and rewriting it for Bunzl is Phase C content work. `governance-room/`'s two
stations are still placeholder codes (`PLACEHOLDER1`/`PLACEHOLDER2`), not yet derived from
anything — real content is Phase D work.
